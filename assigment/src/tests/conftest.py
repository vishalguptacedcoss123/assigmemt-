...
def apply_cli_overrides(pytestconfig):
	"""Apply CLI overrides for environment, base URL, email, and password."""
	from utils.config_manager import config_manager
	
	env_opt = pytestconfig.getoption("--env") if hasattr(pytestconfig, 'getoption') else None
	base_url_opt = pytestconfig.getoption("--base-url") if hasattr(pytestconfig, 'getoption') else None
	email_opt = pytestconfig.getoption("--email") if hasattr(pytestconfig, 'getoption') else None
	password_opt = pytestconfig.getoption("--password") if hasattr(pytestconfig, 'getoption') else None
	...
	logger.info(
		f"Applied CLI overrides: env={env_opt or config_manager.settings.current_env}, "
		f"base_url={base_url_opt or config_manager.get_environment_url()}"
	)
...
def pytest_addoption(parser):
	...
	parser.addoption(
		...
		action="store",
		default=None,
		help="Override Rudderstack email"
	)
	parser.addoption(
		"--password",
		action="store",
		default=None,
		help="Override Rudderstack password"
	)
